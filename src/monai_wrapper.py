from src.network_wrappers import RegistrationModule
...
class ConcatInputs(torch.nn.Module):
    ...
    def forward(self, image_A, image_B):
        return self.net(torch.cat([image_A, image_B], dim=1))
...
def make_ddf_from_icon_transform(transform, identity_map):
    ...
    using an ICON transform. The associated ICON identity_map is also required
    """
...
def make_ddf_using_icon_module(module: RegistrationModule, image_A, image_B):
    """
    Compute A deformation field compatible with monai's Warp
